...
import { Booking, Review, SentimentAnalysis } from '../../types';
...
export const ReviewModal: React.FC<ReviewModalProps> = ({ isOpen, onClose, booking, review, mode, onReviewSubmitted }) => {
  const { currentUser } = useAuth();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedRating, setSelectedRating] = useState(review?.rating || 0);
  const [sentimentPreview, setSentimentPreview] = useState<SentimentAnalysis | null>(null);
  const [showSentimentPreview, setShowSentimentPreview] = useState(false);
  
  const { analyzeText, loading: isAnalyzing, error: sentimentError } = useSentimentAnalysis();
  
  const { register, handleSubmit, formState: { errors }, setValue, reset, watch } = useForm<ReviewForm>({
    defaultValues: {
  ...
  // Real-time sentiment analysis effect
  useEffect(() => {
    ...
    const timeoutId = setTimeout(analyzeComment, 1000); // Debounce 1 second
  ...
  const onSubmit = async (data: ReviewForm) => {
    ...
    try {
      ...
      if (sentimentPreview) {
        reviewData.sentimentAnalysis = sentimentPreview;
        reviewData.sentimentScore = sentimentPreview.score;
        reviewData.sentimentLabel = sentimentPreview.sentiment;
      }
  ...
  // Helper function for sentiment display
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between mb-6">
              <h3 id="review-modal-title" className="text-lg font-medium text-gray-900">{mode === 'edit' ? 'Edit Review' : 'Leave a Review'}</h3>
              <button
                type="button"
                onClick={handleClose}
                  ...
                  {showSentimentPreview && (
                    <div className="mt-3">
                      {isAnalyzing ? (
                        ...
                        <div className={`flex items-center justify-between p-3 border rounded-md ${getSentimentColor(sentimentPreview.sentiment)}`}>
                          <div className="flex items-center space-x-2">
                            <span className="text-lg">{getSentimentIcon(sentimentPreview.sentiment)}</span>
                            <span className="font-medium capitalize">{sentimentPreview.sentiment}</span>
                          </div>
                          <div className="text-sm">
                            {Math.round(sentimentPreview.confidence * 100)}% confidence
                          </div>
                        </div>
                      ) : sentimentError ? (
