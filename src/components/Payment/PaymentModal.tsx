...
import { addDoc, collection, updateDoc, doc, Timestamp, query, where, getDocs, limit } from 'firebase/firestore';
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data.type === 'PAYMENT_SUCCESS') {
        verifyPayment(event.data.txRef);
            ...
            if (popup.closed) {
              setTimeout(() => {
                if (paymentStep === 'processing') {
                  setErrorMessage('Payment window was closed. Please try again.');
                  setPaymentStep('error');
                  setIsProcessing(false);
                }
              }, 2000);
                        ...
                        {paymentMethods.map((method) => (
                          <label
                            ...
                            className={`flex items-center space-x-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors ${
                              selectedPaymentOptions.includes(method.id) ? 'border-amber-500 bg-amber-50' : ''
                            }`}
                          >
                            <input
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 mb-4">
                  {selectedPaymentMethod === 'chapa' ? 
                    ...
                    'Processing your manual payment request...'
                  }
                </p>
                {paymentUrl && selectedPaymentMethod === 'chapa' && (
